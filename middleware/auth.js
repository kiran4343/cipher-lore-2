...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    const admin = await get('SELECT id, email, name, role, is_active FROM admins WHERE id = ?', [decoded.id]);
...
async function requireAuthPage(req, res, next) {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
    const admin = await get('SELECT id, is_active FROM admins WHERE id = ?', [decoded.id]);
    if (!admin || !admin.is_active) {
      res.clearCookie('admin_token');
